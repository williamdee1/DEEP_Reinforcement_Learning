...
class Game:
    ...
    def move_turn(self, move, verbose):
        ...
        # Receiving negative reward for each move taken:
        reward = -1

        # Establishing metrics:
        super_eff = False
        atk_boost_curr = trainer_mon.atk_stage
        verbose_list.append("Current Attack Boost is: %s" % atk_boost_curr)
        ...
        if move.move_cat != 'Status':
            ...
            # Apply status move effect (unlimited potions available):
            sme = status_move_effect(trainer_mon, cpu_mon, move, potions_available=1)

            if sme == 'copy_move':
                #print("Trainer uses mirror move!")
        ...
        if self.cpu_damage_enabled:
            ...
            if cpu_mon.check_fainted() == False:
                ...
                if cpu_move_hits:
                    ...
                    #verbose_list.append("Move accuracy was: %s" % cpu_move.move_acc)
